...
def save_dataframe():
    ...
    print(var_df_final)
    var_df_final.to_csv(save_path, index=False, line_terminator='\n')   # line terminator to avoid empty spaces after rows


# Following 3 lines create the combo box aka drop down list
# Left
combodf1 = Combobox(root, postcommand=update_combo_list1, width=15)
combodf1.set("select")
combodf1.grid(row=2, column=0)
# Right
combodf2 = Combobox(root, postcommand=update_combo_list2, width=15)
combodf2.set("select")
...
left_up = tk.Button(root, text="Dwn", command=move_down1).grid(row=6, column=0)
left_dwn = tk.Button(root, text="Up", command=move_up1).grid(row=7, column=0)
...
right_up = tk.Button(root, text="Dwn", command=move_down2).grid(row=6, column=1)
right_dwn = tk.Button(root, text="Up", command=move_up2).grid(row=7, column=1)
...
key_input = tk.Entry(root, width=15)
key_input.grid(row=9, column=1)

# Button for Merging
merge_btn = tk.Button(root, text="Merge", command=save_dataframe).grid(row=10, column=1)
