...
root.geometry("380x530")    # width x height
...
def add_col(combodfx, dfxcolumns_keep, lb_dfxcols_keep, listbox_x):
...
def add_all(dfxcolumns_keep, lb_dfxcols_keep, listbox_x):
    for x in listbox_x:
...
def del_col(combodfx, dfxcolumns_keep, lb_dfxcols_keep, listbox_x):
...
def del_all(dfxcolumns_keep, lb_dfxcols_keep, listbox_x):
...
def add_col1():
    return add_col(combodf1, df1columns_keep, lb_df1cols_keep, listbox1)


def add_all1():
    return add_all(df1columns_keep, lb_df1cols_keep, listbox1)


def del_col1():
    return del_col(combodf1, df1columns_keep, lb_df1cols_keep, listbox1)


def del_all1():
    return del_all(df1columns_keep, lb_df1cols_keep, listbox1)
...
# lambdas for df2
def add_col2():
    return add_col(combodf2, df2columns_keep, rb_df2cols_keep, listbox2)


def add_all2():
    return add_all(df2columns_keep, rb_df2cols_keep, listbox2)


def del_all2():
    return del_all(df2columns_keep, rb_df2cols_keep, listbox2)

def del_col2():
    return del_col(combodf2, df2columns_keep, rb_df2cols_keep, listbox2)
...
def var_col(d, k):  #Takes merged_df and makes variances columns, k = number of key columns
    ...
    for i in range(k, 1+c):
        ...
        if type(d0.iloc[1, i]) == str:  #Checks to see if the first entry of the column is a string,
            ...
            d0[var_col_name] = d0[first_col_name] - d0[second_col_name]
            print(d0[first_col_name])
            print(d0[second_col_name])
            print(d0[var_col_name])
    print(d0)
    return d0
...
def popupmsg():
    popup = tk.Tk()
    popup.wm_title("Error")     # Title of message
    popup_label = tk.Label(popup, txt="You need to select the same number of columns")
    popup_label.grid()
    close_btn = tk.Button(popup, text="Okay", command=popup.destroy)
    close_btn.grid()
    popup.mainloop()
...
combodf1.set("select")
combodf1.grid(row=3, column=0, sticky='W'+'N'+'S'+'E', padx=(20, 0), pady=5, columnspan=2)
# Right
combodf2 = Combobox(root, postcommand=update_combo_list2, width=20)
combodf2.set("select")
combodf2.grid(row=3, column=3, sticky='W'+'N'+'S'+'E', pady=5, columnspan=2)

# Creates the list boxes
# Left
lb_df1cols_keep = tk.Listbox(root)
lb_df1cols_keep.grid(row=5, column=0, sticky='W'+'N'+'S'+'E', padx=(20, 0), pady=5, columnspan=2)
# Right
rb_df2cols_keep = tk.Listbox(root)
rb_df2cols_keep.grid(row=5, column=3, sticky='W'+'N'+'S'+'E', pady=5, columnspan=2)
...
# Left Buttons
left_add = tk.Button(root, text="Add", command=add_col1, width=10, bg='#A9DFBF').grid(row=4, column=1)
left_del = tk.Button(root, text="Del", command=del_col1, width=10, bg='#F5B7B1').grid(row=4, column=0, padx=(20, 0))
left_dwn = tk.Button(root, text="Dwn", command=move_down1, width=10).grid(row=7, column=0, columnspan=2, padx=(20, 0))
left_up = tk.Button(root, text="Up", command=move_up1, width=10).grid(row=6, column=0, columnspan=2, padx=(20, 0))
left_add_all = tk.Button(root, text="Add All", command=add_all1, width=10, bg='#A9DFBF').grid(row=2, column=1)
left_del_all = tk.Button(root, text="Del All", command=del_all1, width=10, bg='#F5B7B1').grid(row=2, column=0, padx=(20, 0))

# Right Buttons
right_add = tk.Button(root, text="Add", command=add_col2, width=10, bg='#A9DFBF').grid(row=4, column=4)
right_del = tk.Button(root, text="Del", command=del_col2, width=10, bg='#F5B7B1').grid(row=4, column=3)
right_dwn = tk.Button(root, text="Dwn", command=move_down2, width=10).grid(row=7, column=3, columnspan=2)
right_up = tk.Button(root, text="Up", command=move_up2, width=10).grid(row=6, column=3, columnspan=2)
right_add_all = tk.Button(root, text="Add All", command=add_all2, width=10, bg='#A9DFBF').grid(row=2, column=4)
right_del_all = tk.Button(root, text="Del All", command=del_all2, width=10, bg='#F5B7B1').grid(row=2, column=3)

# Adds the text for Number Key Columns
lbl_title = tk.Label(root, text="# Key Cols", bg='#D4E6F1')
lbl_title.grid(row=8, column=1, sticky='W', pady=20, padx=(20, 0))

# Adds free entry field for number of key columns
key_input = tk.Entry(root, width=10)
key_input.grid(row=8, column=2, columnspan=2, sticky='W')


# Button for Merging
merge_btn = tk.Button(root, text="Merge", command=save_dataframe, width=20).grid(row=9, column=1,
                    sticky='W'+'N'+'E'+'S', columnspan=3, padx=(5, 0))

# Button to quit
quit_btn = tk.Button(root, text="Quit", command=close_window, width=20).grid(row=10, column=1,
